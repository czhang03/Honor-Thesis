...
\section{Properties}\label{sec: semi-complete properties}
  ...
  \begin{figure}
    ...
    \tikz\graph[simple necklace layout, math nodes, node sep=2cm] {
      "D_v - T_v" [draw, circle, minimum size = 2cm] ->  %chktex 18 %chktex 8
      v ->
      "S_v - T_v"[draw, circle, minimum size = 2cm];  %chktex 18 %chktex 8
      v <-> T_v [draw, circle, minimum size = 2cm];
    };
  ...
  so adding the \(2_{nd}\) king to the \(k\) kings
  ...
  \begin{proof}
    ...
    and \cref{the: (4 4) digraph exists with one tie}
    provide constructions for these semi-complete digraphs.
