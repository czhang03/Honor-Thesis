...
\begin{proof}
  ...
  Therefore, the only possible beating relationships are
  \(\set{a, b} \to v\) or \(v \to \set{a, b}\).
\end{proof}
...
those vertices that dominate both \(a\) and \(b\):
\(\set{d_1, d_2, d_3}\),
...
we add a vertex \(c\) that ties \(b\) and is adjacent to \(v\)
...
the arrow direction between \(v\) and \(a\) and
the arrow direction between \(v\) and \(b\),
got ``transmitted'' to be the arrow direction between \(v\) and \(c\)
via \cref{the: arrow direction lemma}.
...
\begin{definition}
  ...
  If there are \(n + 1\) vertices in that sequence,
  we will say \keyword{the length of the tie path} is \(n\).
  Note that all the \(a_i\)'s do not have to be distinct.
\end{definition}

Notice that every vertex \(a\) has a tie path \([a]\)
to itself with length 0.
...
Tie paths are useful in analyzing quasi-transitive
oriented graphs:
...
because we still require that vertex \(v\)
be adjacent to the whole tie path.
